  ...
  return (
    <div
      className={`flex items-center p-[3px] cursor-pointer relative ${
        state.variant === 'not-select' ? 'w-8 h-8 rotate-90' : 'h-8 w-8' // Increased size
      ...
      <div
        className={`rounded-sm relative ${
          state.variant === 'hover-2'
            ? 'w-[8px] h-[8px]' // Adjusted for larger size
            : state.variant === 'hover-1'
            ? 'w-[24px] h-[24px]' // Adjusted for larger size
            : 'w-[20px] h-[20px]' // Adjusted for larger size
        } ${
